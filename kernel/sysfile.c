...
/*
 *  sys_read -- read bytes from a file.
 */
int
sys_read (void)
{
...
/*
 *  sys_open -- open or create a new file or pipe.
 */
int
sys_open (void)
{
...
name_for_inode (char * buf, int n, struct inode * ip) 
{
    ...
    {   // namei is inefficient but iget isn't exported for some reason
        buf[0] = '/';
        return 1;
    } 
...
/*
 *  Get the current working directory. This in only working for now because we
 *  have a single terminal session.
 */
int
sys_getcwd (void)
{
