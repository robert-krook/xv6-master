...
uint64 cmos_to_julian () 
{
	uint32 a = (14 - cmos_time.month) / 12;
	uint32 y = (cmos_time.year + 2000) + 4800 - a;
	uint32 m = cmos_time.month + 12 * a - 3;
...
//			cmos_time.day, cmos_time.hour, cmos_time.min, cmos_time.sec);
    ...
    r->year = cmos_time.year + 2000;
    r->month = cmos_time.month;
    r->day = cmos_time.day;
    r->hour = cmos_time.hour;
    r->min = cmos_time.min;
    r->sec = cmos_time.sec;
}
