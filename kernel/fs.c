...
// Fetch number of used blocks
int
get_number_of_used_blocks (uint dev)
{
    int b, bi, m, used_blocks;
    struct buf *bp;
    struct superblock sb;
...
int
get_inodes_used (int max_inodes)
{
  int inum;
  struct buf *bp;
  struct dinode *dip;
...
/*
    Show the disk statistics.

    1) Size of the file system
    2) Number of blocks
    3) Number of inodes
    4) Number of log blocks
    5) Number of bit blocks
    6) Number of used blocks
    7) Number of free blokcs
...
show_disk_free ()
{
    ...
    cprintf ("Disk Free\n");
    cprintf ("\n");
    cprintf ("Size of file system    : %d\n", sb.size);
    cprintf ("Number of blocks       : %d\n", sb.nblocks);
    cprintf ("Number of inodes       : %d\n", sb.ninodes);
    cprintf ("Number of log blocks   : %d\n", sb.nlog);
    cprintf ("Sector size            : %d\n", BSIZE);

    uint bitblocks = sb.size / (BSIZE * 8) + 1;
    cprintf ("Number of bit blocks   : %d\n", bitblocks);

    used_blocks = get_number_of_used_blocks (1);

    cprintf ("Number of used blocks  : %d\n", used_blocks);
    cprintf ("Number of free blocks  : %d\n", sb.size - used_blocks);

    int inodes_used = get_inodes_used (sb.ninodes);
    cprintf ("Number of inodes used  : %d\n", inodes_used);
    cprintf ("Number of inodes free  : %d\n", sb.ninodes - inodes_used);

    cprintf ("Percentage blocks used : %d %%\n", (used_blocks * 100) / sb.size);
    cprintf ("Percentage inodes used : %d %%\n", (inodes_used * 100) /  sb.ninodes);
