...
ifeq ("$(X64)","yes")
	...
	LDFLAGS = -m elf_x86_64 -nostdlib -u _printf_float #-nodefaultlibs 
...
CC = x86_64-elf-gcc #$(TOOLPREFIX)gcc
AS = $(TOOLPREFIX)gas
LD = x86_64-elf-ld #$(TOOLPREFIX)ld
OBJCOPY = /opt/homebrew/opt/llvm/bin/llvm-objcopy #$(TOOLPREFIX)objcopy
OBJDUMP = $(TOOLPREFIX)objdump
CFLAGS = -fno-pic -static -fno-builtin -fno-strict-aliasing -Wall -MD -ggdb -fno-omit-frame-pointer
CFLAGS += -ffreestanding -fno-common -nostdlib -Iinclude -gdwarf-2 $(XFLAGS) $(OPT) -msoft-float -mpreferred-stack-boundary=3
...
out/bootblock: kernel/bootasm.S kernel/bootmain.c
	...
	$(LD) -m elf_i386 -nostdlib -N -e start -Ttext 0x7C00 -o out/bootblock.o out/bootasm.o out/bootmain.o
...
fs/forktest: uobj/forktest.o $(ULIB) $(SHELL_LIB)
	@mkdir -p fs
...
clean: 
	rm -rf out fs uobj kobj shell_obj
...
ifndef CPUS
	CPUS := 8,sockets=1,cores=4,threads=2
endif
...
#	QEMUOPTS = -net none -hda xv6.img -hdb fs.img -smp $(CPUS) -m 2048 $(QEMUEXTRA) -vga std -rtc base=localtime			
#QEMUOPTS = -net none -hda xv6.img -hdb fs.img -smp $(CPUS) -m 512 $(QEMUEXTRA) -vga vmware	
...
qemu-nox: fs.img xv6.img hd.img
...
qemu-nox-gdb: fs.img xv6.img .gdbinit hd.img
	@echo "*** Now run 'gdb'." 1>&2
